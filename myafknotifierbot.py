...
def save_data():
    data_to_save = { "afk_users": {}, "username_to_id": username_to_id }
    for user_id, info in afk_users.items():
        data_to_save["afk_users"][user_id] = {
            "reason": info["reason"], "since": info["since"].isoformat(), "name": info["name"]
        }
...
def load_data():
            ...
            try:
                ...
                for user_id, info in afk_users_raw.items():
                    afk_users[int(user_id)] = {
                        "reason": info["reason"], "since": datetime.fromisoformat(info["since"]), "name": info["name"]
                    }
    ...
    else: logger.info("File data tidak ditemukan, memulai dengan data kosong.")
...
async def set_afk(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if message.entities and message.entities[0].type == MessageEntity.BOT_COMMAND:
        ...
        reason_text = message.text[command_end_offset:].strip()
        if reason_text: reason = reason_text

    afk_users[user.id] = { "reason": reason, "since": datetime.now(timezone.utc), "name": user.first_name }
    ...
    await message.reply_text(f"{safe_name} sekarang AFK\.\n*Alasan:* {safe_reason}", parse_mode='MarkdownV2')
...
async def unset_afk(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user.id in afk_users:
        ...
        if user.username and user.username.lower() in username_to_id: del username_to_id[user.username.lower()]
        save_data()
        
        safe_name = escape_markdown(user.first_name, version=2)
        safe_reason = escape_markdown(reason, version=2)
        await update.message.reply_text(
            f"Selamat datang kembali, {safe_name}\!\n"
            f"Kamu telah AFK selama *{duration_str}*\.\n"
            f"Alasan: {safe_reason}",
            parse_mode='MarkdownV2'
        )
    else: await update.message.reply_text("Kamu memang tidak sedang AFK.")
...
async def check_afk(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if sender_id in afk_users:
        ...
        if sender.username and sender.username.lower() in username_to_id: del username_to_id[sender.username.lower()]
        save_data()
        
        safe_name = escape_markdown(sender.first_name, version=2)
        safe_reason = escape_markdown(reason, version=2)
        await message.reply_text(
            f"{safe_name} kembali online\.\n"
    ...
    async def send_afk_notification(user_id):
        ...
        safe_name = escape_markdown(afk_info['name'], version=2)
        safe_reason = escape_markdown(afk_info['reason'], version=2)
        await message.reply_text(
            f"Pengguna *{safe_name}* sedang AFK\.\n"
            f"Alasan: {safe_reason}\n"
    ...
    if message.reply_to_message:
        if message.reply_to_message.forum_topic_created: return
        replied_user_id = message.reply_to_message.from_user.id
        if replied_user_id in afk_users and replied_user_id != sender_id: await send_afk_notification(replied_user_id)

    if message.entities:
        for entity in message.entities:
            user_id_to_check = None
            if entity.type == MessageEntity.TEXT_MENTION: user_id_to_check = entity.user.id
            ...
            if user_id_to_check and user_id_to_check in afk_users and user_id_to_check != sender_id: await send_afk_notification(user_id_to_check)
